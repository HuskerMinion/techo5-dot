...
for "beamforming", and the order the work should take. Except for [First measurements: the bench
unit](#first-measurements-the-bench-unit-2026-09-15), every number is quoted from EchoLocal (code
comments), EchoMuse (`SETUP.md` and its device notes) or jxlarrea (`docs/echo-cancellation.md`, on
the Echo Show 8 `crown`); that section is this project's own, taken on a real Dot on 2026-09-15.
...
| Channel map | ch 0–5 perimeter ring, ch 6 **center**, ch 7–8 playback loopback L/R | both |
| Geometry | six mics on a **36 mm radius** ring, 60° apart, plus the center (PCB-measured) | EchoMuse, EchoLocal `beam.go` |
...
| Arrival differences | ≈105 µs center to ring (1.7 samples at 16 kHz), ≈210 µs across (3.4 samples) | geometry |
...
- **Amazon's fixed beamformer**, re-implemented from the firmware's coefficient files in
  `/vendor/etc/audio-algorithms/` (`lib/subband`). Two front ends are recognized:
  - `FilterBank_640 + FBF`: 64 bands, 6 beams, all 7 mics.
  - `FilterBank_768cvxGLow + FBFV2`: 128 bands, 8 beams, 4 of 7 mics.
- **Echo cancellation** runs on the **center mic only** and replaces the mix while anything plays.
...
`device/tools/bf_capture`. It works, with a flat response and no artifacts, but at conversational
...
- **Production is a selector**: the center mic for the wake word (the same from every direction),
...
**Geometry works.** GCC-PHAT against the center mic gives clean, sign-consistent lags. Facing
...
| take | center | best single mic | average of 7 | delay-and-sum (measured lags) |
|---|---|---|---|---|
...
The **plain average of all seven beats the center mic in every take**: +0.9 to +2.5 dB overall
...
The average's floor is −78.7 dBFS against the center's −72.2, nearly the 8.5 dB that seven
...
(+1.0 to +2.5 dB relative to the center) don't track that. Either they are already applied
upstream or they mean something else, so they are not used.

**Echo.**
- The center mic lags the loopback by **52 samples (3.25 ms), polarity inverted** (EchoMuse: 33
  on their unit).
...
**Speech over music** (speech frames against the canceled music-only residual):

| front end | 100–8k | <1.5k | 1.5–4.7k |
|---|---|---|---|
| center, no AEC (what a wake word hears today without canceling) | −2.6 | −2.0 | −9.2 |
| center + AEC (the daemon's current design) | 9.1 | 15.3 | −3.3 |
...
center-mic canceller at the same CPU cost**.
...
- microWakeWord `okay_nabu` (cutoff 0.97), full path (mix, cancel, leveler, detector), best
  ...
  - Extra gain ahead of the leveler (×2, ×4, ×8) doesn't move the scores. Level is not the
    limit; the leveler already normalizes.
...
| take | center (old) | average of 7 (new) | false accepts |
|---|---|---|---|
...
2. **Loud music: the linear canceller cannot follow the speaker.**
   ...
   - The residual stays 11 dB over the room floor, and the talker lands at −4.6 dB (center) or
...
3. **Phone speech to one side: this is where spatial filtering earns its place.**
   - Talker windows against between-prompt audio: center **2.0 dB**, average of 7 **2.8 dB**,
...
| engine | moderate music: speech over residual (center / average) | loud music: wake scores |
|---|---|---|
...
| −27 dBFS | −61 dBFS | 15.2 dB | +10.9 dB | 3/5 (avg) / 2/5 (center) | **4/5** |
...
- **The average of seven scores at or above the center mic at every level** (e.g. 0.86/0.70/0.98
...
**Conclusions so far.**
1. Replace "center mic, and a canceled center mic during playback" with **the average of seven,
...
- **Spatial processing has a real but narrow window** (2–4.7 kHz). The two approaches with a chance
  ...
  - **A calibrated superdirective beam limited to that band**, blended with the center mic below
...
- A host tool that replays raw 9-channel captures through every front-end combination in
  `hardware/mic`: center, average, delay-and-sum, Amazon FBF/FBFV2, best-mic selection, AEC
...
- If a spatial stage wins in B3, decide where cancellation goes. Per mic before the beam keeps the
  acoustic path fixed. A fixed beam canceled once is cheaper. With 512 MB and four A53s, seven
  WebRTC instances are likely too heavy.

### B3 — Spatial stage, only if it earns it

Scored on the B0 bench against `Center mic + AEC`, in noisy captures at 3 m:
...
where the center mic fails. Otherwise keep it as a Home Assistant option and say why in this file.
