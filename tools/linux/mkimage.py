...
def main():
    ...
    ap.add_argument("--cmdline-drop", action="append", default=[],
                    ...
                    "(skip_initramfs, root=, dm= for an Android boot image)")
    ...
    if a.ramdisk_file:
        ...
        size = write_bootimg(hdr, kernel, rd, ps, a.output, a.cmdline_append, a.cmdline_drop)
    ...
    if a.minimal:
        ...
        # No password at all rather than an empty one: the console is a shell already, and nothing
        # that asks for a password should ever let root in with none.
